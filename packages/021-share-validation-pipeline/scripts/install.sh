...
heading "Install Package 021"
require_repo; load_env
[[ -n "${SEYMOUR_ENGINE_DATABASE_URL:-}" ]] || fail "Engine database URL"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="$REPO_ROOT/packages/backups/021-share-validation-pipeline-$STAMP"
mkdir -p "$BACKUP_DIR"
...
psql "$SEYMOUR_ENGINE_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$REPO_ROOT/migrations/022_share_validation_pipeline.sql"
pass "Migration 022"
printf '\nPackage 021 installed.\nBackup: %s\n' "$BACKUP_DIR"
