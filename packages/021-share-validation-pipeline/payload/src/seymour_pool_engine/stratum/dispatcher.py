...
class StratumDispatcher:
    def __init__(
        self, repository: StratumRepository, job_service: TemplateJobService | None = None
    ) -> None:
        ...
        if r.method == "mining.submit":
            ...
            try:
                ...
                self.repository.record_validated_submission(
                    s, r.params, accepted=False, reject_reason=exc.reason, share_hash=None,
                    share_difficulty=None, block_candidate=False, header_hex=None,
                )
                return DispatchResult([], (20, exc.message))
            inserted = self.repository.record_validated_submission(
                s, r.params, accepted=result.accepted,
