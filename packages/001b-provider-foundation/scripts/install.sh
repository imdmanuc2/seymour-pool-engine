...
BACKUP_DIR="$REPO_ROOT/packages/backups/001b-provider-foundation-$STAMP"
mkdir -p "$BACKUP_DIR"
...
psql "$SEYMOUR_ENGINE_DATABASE_URL" \
  -v ON_ERROR_STOP=1 \
  -f "$MIGRATION_FILE"
pass "Migration 002"
