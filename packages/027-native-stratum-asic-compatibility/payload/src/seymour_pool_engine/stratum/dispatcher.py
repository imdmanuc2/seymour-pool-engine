...
class StratumDispatcher:
    ...
    def dispatch(self, s: StratumSession, r: RpcRequest) -> DispatchResult:
        ...
        if r.method == "mining.configure":
            ...
            options = (
                r.params[1]
                if len(r.params) > 1 and isinstance(r.params[1], dict)
                else {}
            )
            result = {str(extension): False for extension in requested}

            if "version-rolling" in requested:
                requested_mask = options.get("version-rolling.mask", "ffffffff")
                try:
                    requested_mask_value = int(str(requested_mask), 16)
                except (TypeError, ValueError):
                    requested_mask_value = 0

                negotiated_mask = (
                    requested_mask_value & SUPPORTED_VERSION_ROLLING_MASK
                )
