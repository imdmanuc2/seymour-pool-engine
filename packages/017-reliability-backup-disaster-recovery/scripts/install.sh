...
heading "Install Package 017"; require_repo; load_env
[[ -n "${SEYMOUR_ENGINE_DATABASE_URL:-}" ]] || fail "SEYMOUR_ENGINE_DATABASE_URL"
STAMP="$(date +%Y%m%d-%H%M%S)"; BACKUP_DIR="$REPO_ROOT/packages/backups/017-reliability-backup-disaster-recovery-$STAMP"
...
pass "File backup"; heading "Install package payload"; cp -a "$PAYLOAD_DIR/." "$REPO_ROOT/"; pass "Payload installed"
heading "Install Python project"; PYTHON="$(python_bin)"; "$PYTHON" -m pip install -e "$REPO_ROOT[dev]"; pass "Python project installed"
heading "Apply database migration"; psql "$SEYMOUR_ENGINE_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$MIGRATION_FILE"; pass "Migration 018"
printf '\nPackage 017 installed.\nBackup: %s\n' "$BACKUP_DIR"
