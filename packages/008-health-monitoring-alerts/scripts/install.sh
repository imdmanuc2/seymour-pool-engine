...
heading "Install Package 008"; require_repo; load_env
[[ -n "${SEYMOUR_ENGINE_DATABASE_URL:-}" ]] || fail "SEYMOUR_ENGINE_DATABASE_URL"
command -v psql >/dev/null || fail "PostgreSQL client"
STAMP="$(date +%Y%m%d-%H%M%S)"; BACKUP_DIR="$REPO_ROOT/packages/backups/008-health-monitoring-alerts-$STAMP"
...
heading "Apply database migration"; psql "$SEYMOUR_ENGINE_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$MIGRATION_FILE"; pass "Migration 009"
