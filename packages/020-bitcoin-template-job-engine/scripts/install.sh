...
heading "Install Package 020"
require_repo; load_env
[[ -n "${SEYMOUR_ENGINE_DATABASE_URL:-}" ]] || fail "Engine database URL"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="$REPO_ROOT/packages/backups/020-bitcoin-template-job-engine-$STAMP"
mkdir -p "$BACKUP_DIR"
...
psql "$SEYMOUR_ENGINE_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$REPO_ROOT/migrations/021_bitcoin_template_job_engine.sql"
pass "Migration 021"
printf '\nPackage 020 installed.\nBackup: %s\n' "$BACKUP_DIR"
