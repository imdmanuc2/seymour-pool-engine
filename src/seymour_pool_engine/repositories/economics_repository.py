...
class EconomicsRepository:
        ...
        with engine_connection() as connection, connection.cursor() as cursor:
            ...
            """,
                (
                    ...
                    values["developer_destination"],
                    values["operator_fee_percent"],
    ...
    def get_fee_profile(self, *, pool_id: str, coin: str) -> dict[str, Any] | None:
        with engine_connection() as connection, connection.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM seymour_engine.fee_profiles WHERE pool_id=%s AND coin=%s",
                (pool_id, coin),
            )
            row = cursor.fetchone()
    ...
    def latest_integrity_hash(self, *, pool_id: str, coin: str) -> str | None:
        with engine_connection() as connection, connection.cursor() as cursor:
            cursor.execute(
                """SELECT integrity_hash FROM seymour_engine.reward_calculations
                              WHERE pool_id=%s AND coin=%s ORDER BY created_at DESC LIMIT 1""",
                (pool_id, coin),
            )
            row = cursor.fetchone()
    ...
    def create_calculation(
        ...
        *,
        ...
        developer_fee_percent: Decimal,
        operator_fee_percent: Decimal,
        ...
        operator = sum(
            (x["amount"] for x in allocations if x["allocation_type"] == "operator"), Decimal("0")
        )
        workers = sum(
            (x["amount"] for x in allocations if x["allocation_type"] == "worker"), Decimal("0")
        )
        with engine_connection() as connection, connection.cursor() as cursor:
            cursor.execute(
                """INSERT INTO seymour_engine.reward_calculations (
            ...
            if inserted is None:
                cursor.execute(
                    """
                    SELECT reward_calculation_id, integrity_hash
                    FROM seymour_engine.reward_calculations
                    WHERE block_id=%s
                    """,
                    (block_id,),
                )
                ...
                return {
                    "rewardCalculationId": str(existing["reward_calculation_id"]),
                    "created": False,
                }
            for item in allocations:
                cursor.execute(
                    """INSERT INTO seymour_engine.reward_allocations (
                    ...
                    (
                        ...
                        item["amount"],
                        item["weight"],
                        f"economics:{calculation_id}:{item['allocation_type']}:{item['recipient_key']}",
                        Jsonb({}),
                    ),
                )
            cursor.execute(
                """
                INSERT INTO seymour_engine.reward_calculation_events (
                    reward_calculation_event_id,
                    reward_calculation_id,
                    event_type,
                    integrity_hash,
                    metadata
                )
                VALUES (%s, %s, 'calculated', %s, %s)
                """,
                (uuid4(), calculation_id, integrity_hash, Jsonb({})),
            )
            connection.commit()
    ...
    def economics_summary(self, *, pool_id: str | None, coin: str | None) -> dict[str, Any]:
        ...
        with engine_connection() as connection, connection.cursor() as cursor:
            cursor.execute(
                f"""SELECT COUNT(*) AS calculation_count,
                ...
                FROM seymour_engine.reward_calculations{clause}""",
                tuple(params),
            )
            row = dict(cursor.fetchone())
        return {
            ...
            "developerFees": float(row["developer_fees"]),
            "operatorFees": float(row["operator_fees"]),
            "workerRewards": float(row["worker_rewards"]),
