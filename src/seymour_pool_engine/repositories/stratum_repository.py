import logging
from typing import Any
from uuid import UUID

from psycopg.rows import tuple_row

from seymour_pool_engine.database import engine_connection
from seymour_pool_engine.engines.jobs.models import BitcoinBlockTemplate, StratumJob
from seymour_pool_engine.engines.session.models import StratumSession

logger = logging.getLogger(__name__)


class StratumRepository:
    def _run(self, sql: str, params: tuple[Any, ...] = ()) -> None:
        try:
            with engine_connection() as connection:
                with connection.cursor(row_factory=tuple_row) as cursor:
                    cursor.execute(sql, params)
                connection.commit()
        except Exception:
            logger.exception("Stratum persistence operation failed")

    def create_session(self, s: StratumSession) -> None:
        self._run(
            "INSERT INTO seymour_engine.stratum_sessions "
            "(session_id,remote_host,remote_port,extranonce1,extranonce2_size,difficulty) "
            "VALUES (%s,%s,%s,%s,%s,%s)",
            (
                s.session_id,
                s.remote_host,
                s.remote_port,
                s.extranonce1,
                s.extranonce2_size,
                s.difficulty,
            ),
        )

    def update_session(self, s: StratumSession) -> None:
        self._run(
            "UPDATE seymour_engine.stratum_sessions SET user_agent=%s,worker_name=%s,"
            "subscribed=%s,authorized=%s,last_activity_at=NOW(),"
            "messages_received=%s,messages_sent=%s,submissions_received=%s,"
            "difficulty=%s,accepted_shares=%s,last_share_at=%s,share_interval_ewma=%s,"
            "vardiff_last_retarget_at=%s,vardiff_share_baseline=%s,difficulty_changes=%s "
            "WHERE session_id=%s",
            (
                s.user_agent,
                s.worker_name,
                s.subscribed,
                s.authorized,
                s.messages_received,
                s.messages_sent,
                s.submissions_received,
                s.difficulty,
                s.accepted_shares,
                s.last_share_at,
                s.share_interval_ewma,
                s.vardiff_last_retarget_at,
                s.vardiff_share_baseline,
                s.difficulty_changes,
                s.session_id,
            ),
        )

    def close_session(self, session_id: UUID, reason: str) -> None:
        self._run(
            "UPDATE seymour_engine.stratum_sessions SET disconnected_at=NOW(),disconnect_reason=%s "
            "WHERE session_id=%s",
            (reason, session_id),
        )

    def record_job(self, job: StratumJob) -> None:
        self._run(
            "INSERT INTO seymour_engine.stratum_jobs "
            "(job_id,source,previous_block_hash,coinbase1,coinbase2,merkle_branches,"
            "version,nbits,ntime,clean_jobs,template_height,coinbase_value,"
            "transaction_count,work_id) "
            "VALUES (%s,%s,%s,%s,%s,%s::jsonb,%s,%s,%s,%s,%s,%s,%s,%s) "
            "ON CONFLICT(job_id) DO NOTHING",
            (
                job.job_id,
                job.source,
                job.previous_block_hash,
                job.coinbase1,
                job.coinbase2,
                __import__("json").dumps(list(job.merkle_branches)),
                job.version,
                job.nbits,
                job.ntime,
                job.clean_jobs,
                job.template_height,
                job.coinbase_value,
                job.transaction_count,
                job.work_id,
            ),
        )

    def record_template(self, template: BitcoinBlockTemplate) -> None:
        import json
        from dataclasses import asdict

        raw = {
            "height": template.height,
            "previousblockhash": template.previous_block_hash,
            "version": template.version,
            "bits": template.bits,
            "curtime": template.curtime,
            "mintime": template.mintime,
            "coinbasevalue": template.coinbase_value,
            "target": template.target,
            "workid": template.work_id,
            "longpollid": template.longpoll_id,
            "transactions": [asdict(tx) for tx in template.transactions],
        }
        self._run("UPDATE seymour_engine.bitcoin_block_templates SET active=FALSE WHERE active", ())
        self._run(
            "INSERT INTO seymour_engine.bitcoin_block_templates "
            "(height,previous_block_hash,version,bits,target,curtime,mintime,coinbase_value,"
            "transaction_count,work_id,longpoll_id,raw_template) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb)",
            (
                template.height,
                template.previous_block_hash,
                template.version,
                template.bits,
                template.target,
                template.curtime,
                template.mintime,
                template.coinbase_value,
                len(template.transactions),
                template.work_id,
                template.longpoll_id,
                json.dumps(raw),
            ),
        )

    def get_job(self, job_id: str) -> StratumJob | None:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT job_id,template_height,previous_block_hash,coinbase1,coinbase2,"
                    "merkle_branches,version,nbits,ntime,clean_jobs,source,coinbase_value,"
                    "transaction_count,work_id FROM seymour_engine.stratum_jobs "
                    "WHERE job_id=%s AND active LIMIT 1",
                    (job_id,),
                )
                row = cursor.fetchone()
        if not row:
            return None
        return StratumJob(
            row[0],
            row[1],
            row[2],
            row[3],
            row[4],
            tuple(row[5]),
            row[6],
            row[7],
            row[8],
            row[9],
            row[10],
            row[11],
            row[12],
            row[13],
        )

    def latest_job(self) -> StratumJob | None:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT job_id,template_height,previous_block_hash,coinbase1,coinbase2,"
                    "merkle_branches,version,nbits,ntime,clean_jobs,source,coinbase_value,"
                    "transaction_count,work_id FROM seymour_engine.stratum_jobs "
                    "WHERE active ORDER BY created_at DESC LIMIT 1"
                )
                row = cursor.fetchone()
        if not row:
            return None
        return StratumJob(
            row[0],
            row[1],
            row[2],
            row[3],
            row[4],
            tuple(row[5]),
            row[6],
            row[7],
            row[8],
            row[9],
            row[10],
            row[11],
            row[12],
            row[13],
        )

    def get_template(self, height: int) -> BitcoinBlockTemplate | None:
        import json

        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT raw_template FROM seymour_engine.bitcoin_block_templates "
                    "WHERE height=%s ORDER BY received_at DESC LIMIT 1",
                    (height,),
                )
                row = cursor.fetchone()
        if not row:
            return None
        payload = row[0]
        if isinstance(payload, str):
            payload = json.loads(payload)
        return BitcoinBlockTemplate.from_rpc(payload)

    def template_status(self) -> dict[str, Any]:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT height,previous_block_hash,transaction_count,"
                    "coinbase_value,received_at "
                    "FROM seymour_engine.bitcoin_block_templates WHERE active "
                    "ORDER BY received_at DESC LIMIT 1"
                )
                row = cursor.fetchone()
        if not row:
            return {"status": "empty", "template": None}
        return {
            "status": "ready",
            "template": {
                "height": row[0],
                "previousBlockHash": row[1],
                "transactionCount": row[2],
                "coinbaseValue": row[3],
                "receivedAt": row[4],
            },
        }

    def record_validated_submission(
        self,
        s: StratumSession,
        params: list[Any],
        *,
        accepted: bool,
        reject_reason: str | None,
        share_hash: str | None,
        share_difficulty: float | None,
        block_candidate: bool,
        header_hex: str | None,
    ) -> bool:
        values = params + [None] * 5
        fingerprint = ":".join(str(value) for value in values[:5])
        try:
            with engine_connection() as connection:
                with connection.cursor(row_factory=tuple_row) as cursor:
                    cursor.execute(
                        "INSERT INTO seymour_engine.stratum_submissions "
                        "(session_id,worker_name,job_id,extranonce2,ntime,nonce,accepted,"
                        "reject_reason,validation_mode,submission_fingerprint,share_hash,"
                        "share_difficulty,block_candidate,header_hex) "
                        "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,'sha256d-full',%s,%s,%s,%s,%s) "
                        "ON CONFLICT(submission_fingerprint) DO NOTHING",
                        (
                            s.session_id,
                            s.worker_name,
                            values[1],
                            values[2],
                            values[3],
                            values[4],
                            accepted,
                            reject_reason,
                            fingerprint,
                            share_hash,
                            share_difficulty,
                            block_candidate,
                            header_hex,
                        ),
                    )
                    inserted = cursor.rowcount == 1
                connection.commit()
            return inserted
        except Exception:
            logger.exception("Stratum submission persistence failed")
            raise


    def record_difficulty_change(
        self,
        s: StratumSession,
        old_difficulty: float,
        observed_share_seconds: float | None,
        target_share_seconds: float,
        reason: str,
    ) -> None:
        self._run(
            "INSERT INTO seymour_engine.stratum_difficulty_history "
            "(session_id,worker_name,old_difficulty,new_difficulty,"
            "observed_share_seconds,target_share_seconds,reason) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s)",
            (
                s.session_id,
                s.worker_name,
                old_difficulty,
                s.difficulty,
                observed_share_seconds,
                target_share_seconds,
                reason,
            ),
        )

    def vardiff_status(self) -> dict[str, Any]:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT COUNT(*) FILTER(WHERE disconnected_at IS NULL),"
                    "COALESCE(AVG(difficulty) FILTER(WHERE disconnected_at IS NULL),0),"
                    "COALESCE(MIN(difficulty) FILTER(WHERE disconnected_at IS NULL),0),"
                    "COALESCE(MAX(difficulty) FILTER(WHERE disconnected_at IS NULL),0),"
                    "COALESCE(SUM(difficulty_changes),0) "
                    "FROM seymour_engine.stratum_sessions"
                )
                row = cursor.fetchone()
        return {
            "activeSessions": row[0],
            "averageDifficulty": float(row[1]),
            "minimumDifficulty": float(row[2]),
            "maximumDifficulty": float(row[3]),
            "difficultyChanges": row[4],
        }

    def difficulty_history(self, limit: int) -> list[dict[str, Any]]:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT difficulty_event_id,session_id,worker_name,old_difficulty,"
                    "new_difficulty,observed_share_seconds,target_share_seconds,reason,created_at "
                    "FROM seymour_engine.stratum_difficulty_history "
                    "ORDER BY created_at DESC LIMIT %s",
                    (limit,),
                )
                rows = cursor.fetchall()
        return [
            {
                "difficultyEventId": r[0],
                "sessionId": str(r[1]) if r[1] else None,
                "workerName": r[2],
                "oldDifficulty": float(r[3]),
                "newDifficulty": float(r[4]),
                "observedShareSeconds": r[5],
                "targetShareSeconds": r[6],
                "reason": r[7],
                "createdAt": r[8],
            }
            for r in rows
        ]
    def submission_stats(self) -> dict[str, Any]:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT COUNT(*),COUNT(*) FILTER(WHERE accepted),"
                    "COUNT(*) FILTER(WHERE NOT accepted),"
                    "COUNT(*) FILTER(WHERE block_candidate),"
                    "MAX(created_at) FROM seymour_engine.stratum_submissions"
                )
                row = cursor.fetchone()
        return {
            "total": row[0],
            "accepted": row[1],
            "rejected": row[2],
            "blockCandidates": row[3],
            "lastSubmissionAt": row[4],
        }

    def status(self) -> dict[str, Any]:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT COUNT(*) FILTER(WHERE disconnected_at IS NULL),"
                    "COUNT(*) FILTER(WHERE disconnected_at IS NULL AND authorized),"
                    "COALESCE(SUM(submissions_received),0) FROM seymour_engine.stratum_sessions"
                )
                row = cursor.fetchone()
        return {
            "activeSessions": row[0],
            "authorizedSessions": row[1],
            "submissions": row[2],
        }

    def sessions(self, limit: int) -> list[dict[str, Any]]:
        with engine_connection() as connection:
            with connection.cursor(row_factory=tuple_row) as cursor:
                cursor.execute(
                    "SELECT session_id,remote_host,remote_port,worker_name,subscribed,"
                    "authorized,difficulty,connected_at,last_activity_at,disconnected_at "
                    "FROM seymour_engine.stratum_sessions ORDER BY connected_at DESC LIMIT %s",
                    (limit,),
                )
                rows = cursor.fetchall()
        return [
            {
                "sessionId": str(r[0]),
                "remoteHost": r[1],
                "remotePort": r[2],
                "workerName": r[3],
                "subscribed": r[4],
                "authorized": r[5],
                "difficulty": float(r[6]),
                "connectedAt": r[7],
                "lastActivityAt": r[8],
                "disconnectedAt": r[9],
            }
            for r in rows
        ]
