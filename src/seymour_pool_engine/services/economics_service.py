...
class EconomicsService:
    ...
    def configure_fee_profile(
        ...
        *,
        ...
        developer_destination: str,
        operator_fee_percent: Decimal = Decimal("0"),
        ...
        if operator_fee_percent > 0 and not operator_destination:
            raise ValueError(
                "Operator destination is required when operator fee is greater than zero"
            )
    ...
    def calculate(
        ...
        *,
        ...
        worker_weights: list[dict[str, Any]],
    ) -> dict[str, Any]:
        ...
        allocations = self._allocate(
            ...
            developer_destination=profile["developer_destination"],
            operator_destination=profile.get("operator_destination"),
        ...
        result = self.repository.create_calculation(
            ...
            operator_fee_percent=operator_percent,
            allocations=allocations,
    ...
    def summary(self, *, pool_id: str | None = None, coin: str | None = None) -> dict[str, Any]:
        return self.repository.economics_summary(
            pool_id=pool_id, coin=coin.upper() if coin else None
        )
    ...
    def _allocate(
        *,
        gross_reward: Decimal,
        developer_percent: Decimal,
        operator_percent: Decimal,
        developer_destination: str,
        operator_destination: str | None,
        worker_weights: list[dict[str, Any]],
        policy: str,
    ) -> list[dict[str, Any]]:
        ...
        developer_amount = (gross_reward * developer_percent / 100).quantize(
            COIN_QUANTUM, rounding=ROUND_DOWN
        )
        operator_amount = (gross_reward * operator_percent / 100).quantize(
            COIN_QUANTUM, rounding=ROUND_DOWN
        )
        worker_total = gross_reward - developer_amount - operator_amount
        rows = [
            {
                "allocation_type": "developer",
                "recipient_key": developer_destination,
            ...
            rows.append(
                {
                    ...
                    "recipient_key": operator_destination,
        ...
        for index, worker in enumerate(cleaned):
            amount = (
                worker_total - allocated
                if index == len(cleaned) - 1
                else (worker_total * worker["weight"] / total_weight).quantize(
                    COIN_QUANTUM, rounding=ROUND_DOWN
                )
            )
            allocated += amount
            rows.append(
                {
                    ...
                    "recipient_key": worker["recipient_key"],
                    "amount": amount,
    ...
    def _serialize_profile(row: dict[str, Any]) -> dict[str, Any]:
        return {
            "feeProfileId": str(row["fee_profile_id"]),
            "poolId": row["pool_id"],
            "coin": row["coin"],
    ...
    def _serialize_allocation(item: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "recipientKey": item["recipient_key"],
