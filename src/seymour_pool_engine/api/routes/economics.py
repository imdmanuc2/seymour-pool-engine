...
def calculate(request: EconomicsCalculationRequest) -> dict[str, Any]:
    try:
        ...
        values["worker_weights"] = [
            {"recipientKey": x["recipient_key"], "weight": x["weight"]}
            for x in values["worker_weights"]
        ]
...
def summary(
    ...
    coin: Annotated[str | None, Query()] = None,
) -> dict[str, Any]:
    try:
...
def policy() -> dict[str, Any]:
    return {
        "developerFeeRequired": True,
        "minimumDeveloperFeePercent": 0.75,
        "operatorMayIncreaseDeveloperFee": True,
        "operatorMayDisableDeveloperFee": False,
